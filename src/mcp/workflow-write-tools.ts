...
import { type AppConfig, assertProjectWriteAllowed } from "../config.js";
...
async function runNamedUpdate(
  ...
  assertProjectWriteAllowed(options.config, projectId);
...
export function registerWorkflowWriteTools(options: RegisterWorkflowWriteToolsOptions): void {
  options.server.registerTool(
    ...
    async ({ owner, number, itemId, status }) => runNamedUpdate(options, {
      owner,
      number,
      itemId,
      optionName: status,
  ...
  options.server.registerTool(
    ...
    async ({ owner, number, itemId, priority }) => runNamedUpdate(options, {
      owner,
      number,
      itemId,
      optionName: priority,
  ...
  options.server.registerTool(
    ...
    async ({ limit, projectId, itemId }) => options.json({
      persistence: "process-local",
      entries: writeAuditLog.list({ limit, projectId, itemId }),
    }),
  );
}
